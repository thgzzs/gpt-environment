...
export function createSky(camera) {
  ...
  function render(ctx, time) {
    ...
    let horizonColor = tonedDownHorizon(
      (dayFactor <= 0.33) ? lerpColor(nightHorizonColor, sunriseColor, smoothStep(0,0.33,dayFactor))
                          : dayFactor <= 0.66 ? lerpColor(sunriseColor, softPurple, smoothStep(0.33,0.66,dayFactor))
                          : lerpColor(softPurple, skyBottomColor, smoothStep(0.66,1,dayFactor)),
      skyBottomColor,
      0.4
    );
    ...
    const right = { x: Math.sin(camera.yaw - Math.PI/2), y:0, z: Math.cos(camera.yaw - Math.PI/2) };
    const up = {
    ...
    function fadeToBlack(color, alpha) {
      return { r: Math.round(color.r * (1 - alpha)), g: Math.round(color.g * (1 - alpha)), b: Math.round(color.b * (1 - alpha)) };
    }
